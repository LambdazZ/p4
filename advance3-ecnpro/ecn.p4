...
const bit<32> CPU_PACKET_HEADER_LENGTH = 26;
...
control MyEgress(inout headers hdr,
    ...
    apply {
        ...
        if (standard_metadata.instance_type == PKT_INSTANCE_TYPE_EGRESS_CLONE) {
            ...
            hdr.cpu.egress_port = meta.egress_port;
            hdr.cpu.ecn_threshold = meta.ecn_threshold;
            hdr.ethernet.etherType = TYPE_CPU;
            hdr.ipv4.setInvalid();
            truncate(CPU_PACKET_HEADER_LENGTH); //ether+cpu header
        }
        else {
            judge_congestion.apply();

            meta.egress_port = standard_metadata.egress_port;
            bit<19> threshold = meta.ecn_threshold;
            if (threshold == 0) {
                threshold = ECN_THRESHOLD_DEFAULT;
                meta.ecn_threshold = threshold;
            }
